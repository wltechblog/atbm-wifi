...
struct bt_mesh_net {
	...
	struct bt_mesh_app_key p_app_keys[MYNEWT_VAL(BLE_MESH_PROVISIONER_APP_KEY_COUNT)];

	/* Next app_idx can be assigned */
	u16_t p_app_idx_next;

	/* Network keys stored by provisioner */
	struct bt_mesh_subnet p_sub[MYNEWT_VAL(BLE_MESH_PROVISIONER_SUBNET_COUNT)];
