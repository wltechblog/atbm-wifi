...
static void
ble_hs_hci_cmd_body_auth_pyld_tmo(uint16_t handle, uint16_t auth_pyld_tmo, uint8_t *dst)
{
    put_le16(dst + 0, handle);
	put_le16(dst + 2, auth_pyld_tmo);
}


void ble_hs_hci_cmd_build_auth_pyld_tmo(uint16_t handle, uint16_t auth_pyld_tmo,
                                uint8_t *dst, int dst_len)
{
	BLE_HS_DBG_ASSERT(dst_len >= BLE_HCI_WR_AUTH_PYLD_TMO_LEN+2);

	ble_hs_hci_cmd_body_auth_pyld_tmo(handle,auth_pyld_tmo,dst);
