...
ble_hs_dbg_le_event_disp(uint8_t subev, uint8_t len, uint8_t *evdata)
{
    ...
    case BLE_HCI_LE_SUBEV_CONN_COMPLETE:
        ...
        if (status == BLE_ERR_SUCCESS) {
            BLE_HS_LOG(DEBUG, "LE conn complete. hdl=%u role=%u "
                              "paddtype=%u addr=%x.%x.%x.%x.%x.%x ",
            ...
            if (subev == BLE_HCI_LE_SUBEV_ENH_CONN_COMPLETE) {
                BLE_HS_LOG(DEBUG, "l_rpa=%x.%x.%x.%x.%x.%x "
                                   "p_rpa=%x.%x.%x.%x.%x.%x ",
    ...
    case BLE_HCI_LE_SUBEV_ADV_RPT:
        ...
        BLE_HS_LOG(DEBUG, "LE adve report. len=%u num=%u evtype=%u "
    ...
    case BLE_HCI_LE_SUBEV_CONN_UPD_COMPLETE:
        status = evdata[0];
        if (status == BLE_ERR_SUCCESS) {
            BLE_HS_LOG(DEBUG, "LE Conn Update Complete. hdl=%u "
                              "itvl=%u latency=%u timeout=%u\n",
                       get_le16(evdata + 1), get_le16(evdata + 3),
                       get_le16(evdata + 5), get_le16(evdata + 7));
        } else {
            BLE_HS_LOG(DEBUG, "LE Conn Update Complete. FAIL "
                              "(status=%u)\n", status);
        }
        break;

    case BLE_HCI_LE_SUBEV_DATA_LEN_CHG:
        BLE_HS_LOG(DEBUG, "LE Data Len Change. hdl=%u max_tx_bytes=%u "
    ...
    case BLE_HCI_LE_SUBEV_REM_CONN_PARM_REQ:
        BLE_HS_LOG(DEBUG, "LE Remote Conn Param Req. hdl=%u "
    ...
    case BLE_HCI_LE_SUBEV_RD_REM_USED_FEAT:
        status = evdata[0];
        if (status == BLE_ERR_SUCCESS) {
            BLE_HS_LOG(DEBUG, "LE Rem Used Features. hdl=%u feat=",
                       get_le16(evdata + 1));
            ...
            BLE_HS_LOG(DEBUG, "LE Rem Used Features. FAIL (status=%u)\n",
                       status);
        }
        break;

    case BLE_HCI_LE_SUBEV_LT_KEY_REQ:
            BLE_HS_LOG(DEBUG, "LE LTK Req. hdl=%u rand=%x%x encdiv=%u\n",
                       get_le16(evdata), get_le32(evdata + 6),
                       get_le32(evdata + 2), get_le16(evdata + 10));
        break;

    case BLE_HCI_LE_SUBEV_PHY_UPDATE_COMPLETE:
            BLE_HS_LOG(DEBUG, "PHY update. hdl=%u tx=%u rx=%u\n",
                       get_le16(evdata + 1), evdata[3], evdata[4]);
        break;
    ...
    case BLE_HCI_LE_SUBEV_DIRECT_ADV_RPT:
    {
        ...
        if (len < sizeof(*data) ||
            ...
            BLE_HS_LOG(DEBUG, "Cp LE Directed Adv Report "
                       "len=%u\n", len);
            break;
        }

        BLE_HS_LOG(DEBUG, "LE Directed Adv Report len=%u "
    ...
    case BLE_HCI_LE_SUBEV_RD_LOC_P256_PUBKEY:
    {
        ...
        if (len != sizeof(*data)) {
            BLE_HS_LOG(DEBUG, "Cp LE Read Loc P-256 PubKey "
                       "Complete Event len=%u\n", len);
            break;
        }

        BLE_HS_LOG(DEBUG, "LE Rd Loc P-256 PubKey Complete "
    ...
    case BLE_HCI_LE_SUBEV_GEN_DHKEY_COMPLETE:
    {
        ...
        if (len != sizeof(*data)) {
            BLE_HS_LOG(DEBUG, "Cp LE DHKey end "
                       "len=%u\n", len);
            break;
        }
    ...
    case BLE_HCI_LE_SUBEV_EXT_ADV_RPT:
    {
        ...
        if (len < sizeof(*data) + sizeof(*params)) {
            BLE_HS_LOG(DEBUG, "Cp LE Extended adv Report "
                       "len=%u\n", len);
            break;
        }

        BLE_HS_LOG(DEBUG, "LE ExAdv Report len=%u num=0x%02x ",
    ...
    case BLE_HCI_LE_SUBEV_PER_ADV_SYNC_ESTAB:
    {
        ...
        if (len != sizeof(*data)) {
            BLE_HS_LOG(DEBUG, "Cp LE perAdv Sync "
                       "Established len=%u\n", len);
            break;
        }

        BLE_HS_LOG(DEBUG, "LE perAdv Sync Established "
                   "len=%u status=0x%02x hdl=0x%04x", len, data->status,
    ...
    case BLE_HCI_LE_SUBEV_PER_ADV_RPT:
    {
        ...
        if (len < sizeof(*data) || len != sizeof(*data) + data->data_length) {
            BLE_HS_LOG(DEBUG, "Cp LE perAdv Report "
                    "len=%u\n", len);
            break;
        }

        BLE_HS_LOG(DEBUG, "LE perAdv Report "
                   "len=%u hdl=0x%04x data_status=0x%02x data_len=0x%02x",
    ...
    case BLE_HCI_LE_SUBEV_PER_ADV_SYNC_LOST:
    {
        ...
        if (len != sizeof(*data)) {
            BLE_HS_LOG(DEBUG, "Cp LE perAdv Sync Lost "
                       "len=%u\n", len);
            break;
        }

        BLE_HS_LOG(DEBUG, "LE perAdv Sync Lost "
                   "len=%u hdl=0x%04x", len, le16toh(data->sync_handle));
        break;
    }

    case BLE_HCI_LE_SUBEV_SCAN_TIMEOUT:
        if (len) {
            BLE_HS_LOG(DEBUG, "Cp LE Scan Timeout len=%u", len);
            break;
        }
    ...
    case BLE_HCI_LE_SUBEV_ADV_SET_TERMINATED:
    {
        ...
        if (len != sizeof(*data)) {
            BLE_HS_LOG(DEBUG, "Cp LE adv Set Terminated "
                       "len=%u\n", len);
            break;
        }

        BLE_HS_LOG(DEBUG, "LE adv Set Terminated len=%u "
                   "status=0x%02x adv=0x%02x conn=0x%04x "
    ...
    case BLE_HCI_LE_SUBEV_SCAN_REQ_RCVD:
    {
        ...
        if (len != sizeof(*data)) {
            BLE_HS_LOG(DEBUG, "Cp LE Scan Request Received "
                       "len=%u\n", len);
            break;
        }

        BLE_HS_LOG(DEBUG, "LE Scan Request rx len=%u "
    ...
    case BLE_HCI_LE_SUBEV_CHAN_SEL_ALG:
    {
        ...
        if (len != sizeof(*data)) {
            BLE_HS_LOG(DEBUG, "Cp LE Channel Selection Algorithm "
                       "len=%u\n", len);
            break;
        }
...
ble_hs_dbg_disconn_comp_disp(uint8_t *evdata, uint8_t len)
{
    ...
    BLE_HS_LOG(DEBUG, "Disconn: status=%u hdl=%u "
...
ble_hs_dbg_encrypt_chg_disp(uint8_t *evdata, uint8_t len)
{
    ...
    BLE_HS_LOG(DEBUG, "Encrypt change: status=%u hdl=%u state=%u\n",
...
ble_hs_dbg_encrypt_refresh_disp(uint8_t *evdata, uint8_t len)
{
    ...
    BLE_HS_LOG(DEBUG, "key refresh: status=%u hdl=%u\n",
               status, handle);
}
...
{
    BLE_HS_LOG(DEBUG, "Rem: status=%u hdl=%u vers_nr=%u "
                      "compid=%u subver=%u\n",
...
ble_hs_dbg_num_comp_pkts_disp(uint8_t *evdata, uint8_t len)
{
    ...
    handles = evdata[0];
    if (len != ((handles * 4) + 1)) {
        BLE_HS_LOG(DEBUG, "ERR: Num of Completed Pkts bad len: "
                          "num_handles=%u len=%u\n", handles, len);
        return;

    }

    BLE_HS_LOG(DEBUG, "Num of Completed Pkts: num_handles=%u\n",
               handles);
    if (handles) {
        handle_ptr = evdata + 1;
        while (handles) {
...
ble_hs_dbg_auth_pyld_tmo_disp(uint8_t *evdata, uint8_t len)
{
    ...
    if (len != sizeof(uint16_t)) {
        BLE_HS_LOG(DEBUG, "ERR: AuthPyldTmoEvent bad len %u\n", len);
        return;
...
ble_hs_dbg_cmd_complete_disp(uint8_t *evdata, uint8_t len)
{
    ...
    if (len < 3) {
        BLE_HS_LOG(DEBUG, "Invalid cmd complete: len=%d "
                          "(expected >= 3)", len);
        goto done;
    }
    ...
    BLE_HS_LOG(DEBUG, "Cmd complete: pkts=%u ogf=0x%x ocf=0x%x",
               cmd_pkts, ogf, ocf);
    ...
    case BLE_HCI_OGF_LE:
        ...
        case BLE_HCI_OCF_LE_RD_CHAN_MAP:
            BLE_HS_LOG(DEBUG, "hdl=%u chanmap=%x.%x.%x.%x.%x",
...
ble_hs_dbg_cmd_status_disp(uint8_t *evdata, uint8_t len)
{
    ...
    BLE_HS_LOG(DEBUG, "Cmd Status: status=%u cmd_pkts=%u ocf=0x%x "
...
{
#if 0//(MYNEWT_VAL_LOG_LEVEL) > LOG_LEVEL_DEBUG || BLE_MONITOR
    return;
#endif
