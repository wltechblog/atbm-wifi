...
SRAM_CODE void ble_store_ram_print_value_sec(const struct ble_store_value_sec *sec)
{
    if (sec->ltk_present) {
...
SRAM_CODE int ble_store_ram_find_sec(const struct ble_store_key_sec *key_sec,
...
SRAM_CODE int ble_store_ram_find_cccd(const struct ble_store_key_cccd *key)
{
...
SRAM_CODE int ble_store_ram_delete_cccd(const struct ble_store_key_cccd *key_cccd)
{
    int idx;
    int rc;
...
SRAM_CODE int ble_store_ram_read_cccd(const struct ble_store_key_cccd *key_cccd,
...
SRAM_CODE int ble_store_ram_write_cccd(const struct ble_store_value_cccd *value_cccd)
{
...
SRAM_CODE void ble_store_ram_init(void)
{
